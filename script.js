document.addEventListener('DOMContentLoaded', () => {
    ...
    if (fromAyahInput) fromAyahInput.addEventListener('input', validateAyahRange);
    if (toAyahInput) toAyahInput.addEventListener('input', validateAyahRange);
    if (modalSurahInput) modalSurahInput.addEventListener('input', validateAyahRange);
    ...
    function setAudioSource() {
        if (!audioPlayer || !reciterSelect || !surahSelect) return;
        audioPlayer.pause();
        audioPlayer.src = reciterSelect.value + surahSelect.value + ".mp3";
        fetchAndDisplaySurahText(surahSelect.value);
    }

    if (surahSelect) surahSelect.addEventListener('change', () => { setAudioSource(); if(playPauseBtn) playPauseBtn.innerText = "بدء"; });
    if (reciterSelect) reciterSelect.addEventListener('change', () => { setAudioSource(); if(playPauseBtn) playPauseBtn.innerText = "بدء"; });

    window.togglePlay = function() {
        if (!audioPlayer) return;
        if (!audioPlayer.src || audioPlayer.src === window.location.href) { setAudioSource(); }
        if (audioPlayer.paused) {
            audioPlayer.play().then(() => { if(playPauseBtn) playPauseBtn.innerText = "توقف"; }).catch(err => { console.error(err); });
        } else { 
            audioPlayer.pause(); 
            if(playPauseBtn) playPauseBtn.innerText = "بدء"; 
        }
    };
    ...
    window.increment = function() { 
        ...
        if (c) c.innerText = parseInt(c.innerText || '0') + 1; 
    };
    ...
    window.setShareType = function(type) {
        ...
        if (type === 'image' || type === 'text') {
            if (reciterWrapper) reciterWrapper.style.display = 'none'; 
            if (creditText) creditText.style.display = 'none';
        } else {
            if (reciterWrapper) reciterWrapper.style.display = 'block';
            if (creditText) creditText.style.display = 'none';
        }
    };
